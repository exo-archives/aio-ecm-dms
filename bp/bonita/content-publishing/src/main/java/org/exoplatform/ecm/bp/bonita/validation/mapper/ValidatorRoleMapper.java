...
public class ValidatorRoleMapper implements RoleMapper {
  ...
  public Set<String> searchMembers(QueryAPIAccessor readonlyapiaccessor, ProcessInstanceUUID instanceId, String roleId) {
	...
	try {
		roleName = (String) readonlyapiaccessor.getQueryRuntimeAPI().getProcessInstanceVariable(instanceId, PROPERTY_NAME);
		delegator = (String) readonlyapiaccessor.getQueryRuntimeAPI().getProcessInstanceVariable(instanceId, DELEGATOR_NAME);
		delegate = (Boolean) readonlyapiaccessor.getQueryRuntimeAPI().getProcessInstanceVariable(instanceId, DELEGATE_NAME);
	...
	if(candidats.size()<2) return candidats;
	if(delegate){
