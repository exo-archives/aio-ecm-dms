...
public class TestNodeService extends BaseECMTestCase {
  ...
  final private String NT_UNSTRUCTURED = "nt:unstructured" ;
  final private String NT_FOLDER = "nt:folder" ;
  ...
  public void testAddNode() throws Exception {
    ...
    //node case
    JcrItemInput jcrItemInputNode = createNodeInput("nt:unstructured", "mix:votable", "NodeName","/node") ;
    ...
    //properties case
    JcrItemInput jcrItemInputProperties = createPropertyInput("/node/exo:votable", "PropValue") ;
  ...
  public void testAddNodeNTFileAndArticle() throws Exception{
    ...
    Map<String , JcrItemInput> mapFile = prepareFileProperties("myFile", "text/abc", new GregorianCalendar());
    ...
    assertEquals(mimeType, "text/abc") ;    
  ...
  public void testAddHierarchicalNode() throws Exception {
    SessionProvider sessionProvider = sessionProviderService_.getSystemSessionProvider(null) ;
    ManageableRepository repository = repositoryService.getRepository(REPO_NAME);
    Session session = sessionProvider.getSession(COLLABORATION_WS, repository) ;
    Node rootNode = session.getRootNode() ;
    Node testNode = rootNode.addNode("test",NT_UNSTRUCTURED) ;
    session.save() ;
    Map<String, JcrItemInput> map = prepareHierachicalNodeProperties() ;
    nodeService_.addNode(testNode, NT_UNSTRUCTURED, map, true) ;
    assertNotNull(testNode.getNode("root")) ;
    assertNotNull(testNode.getNode("root/child1")) ;
    assertNotNull(testNode.getNode("root/child2")) ;
    assertNotNull(testNode.getNode("root/child1/child11")) ;
    assertNotNull(testNode.getNode("root/child2/child21")) ;
    assertNotNull(testNode.getNode("root/child2/myfile")) ;
    assertNotNull(testNode.getNode("root/child2/myfile/jcr:content")) ;
    assertNotNull(testNode.getProperty("root/child2/myfile/jcr:content/jcr:mimeType")) ;
    testNode.remove() ;
    session.save() ;
  }
  ...
  private Node createTestNode(String nodeName, String repoString, String nodeType,
    ...
    //node case
    jcrItemInputNode = createNodeInput(nodeType, "mix:votable", nodeName,"/node") ;
    map.put("/node", jcrItemInputNode) ; 

    //properties case
    jcrItemInputProperties = createPropertyInput("/node/exo:title", "PropertiesCaseTest") ;
    map.put("/node/exo:title", jcrItemInputProperties) ;
  ...
  private Map<String, JcrItemInput> prepareHierachicalNodeProperties() {
    Map<String, JcrItemInput> map = new HashMap<String, JcrItemInput>();
    JcrItemInput root = createNodeInput(NT_UNSTRUCTURED, "mix:votable", "root","/node");
    map.put("/node", root) ;
    JcrItemInput child1 = createNodeInput(NT_UNSTRUCTURED, "mix:votable", "child1","/node/child1");    
    map.put("/node/child1", child1) ;

    JcrItemInput child11 = createNodeInput(NT_UNSTRUCTURED, "mix:votable", "child11","/node/child1/child11");
    map.put("/node/child1/child11", child11) ;

    JcrItemInput child2 = createNodeInput(NT_UNSTRUCTURED, "mix:votable", "child2","/node/child2");
    map.put("/node/child2", child2) ;
    
    JcrItemInput child21 = createNodeInput(NT_UNSTRUCTURED, "mix:votable", "child21","/node/child2/child21");
    map.put("/node/child2/child21", child21) ;
    
//  Node - nt:file
    JcrItemInput itemInput = createNodeInput("nt:file", "mix:votable", "myfile","/node/child2/myfile");
    map.put("/node/child2/myfile", itemInput);

    //Node - jcr:content
    JcrItemInput itemInputJcrContent = createNodeInput("nt:resource", null, "jcr:content","/node/child2/myfile/jcr:content");
    map.put("/node/child2/myfile/jcr:content", itemInputJcrContent);

    //properties - data
    JcrItemInput itemInputData = createPropertyInput("/node/child2/myfile/jcr:content/jcr:data", "");
    map.put("/node/child2/myfile/jcr:content/jcr:data", itemInputData);

    //properties - mimetype
    //mimetype can be: text/html,text/plain....
    JcrItemInput itemInputMimeType = createPropertyInput("/node/child2/myfile/jcr:content/jcr:mimeType", "text");
    map.put("/node/child2/myfile/jcr:content/jcr:mimeType", itemInputMimeType);

    //properties - last modified
    JcrItemInput itemInputLastMdf = createPropertyInput("/node/child2/myfile/jcr:content/jcr:lastModified", new GregorianCalendar());
    map.put("/node/child2/myfile/jcr:content/jcr:lastModified", itemInputLastMdf);
    
    return map ;
  }
  private Map<String, JcrItemInput> prepareArticleProperties() {
    Map<String, JcrItemInput> map = new HashMap<String, JcrItemInput>();
    JcrItemInput itemInput = createNodeInput("exo:article", "mix:votable", "MyArticle","/node");
    map.put("/node", itemInput);

    JcrItemInput itemInputPropText = createPropertyInput("/node/exo:text", "MyText");
    map.put("/node/exo:text", itemInputPropText);

    JcrItemInput itemInputPropTitle = createPropertyInput("/node/exo:title", "MyTitle");
    map.put("/node/exo:title", itemInputPropTitle);

    JcrItemInput itemInputPropSummary = createPropertyInput("/node/exo:summary", "MySummary");
  ...
  private Map<String, JcrItemInput> prepareFileProperties(String fileName,
    ...
    //Node - nt:file
    JcrItemInput itemInput = createNodeInput("nt:file", "mix:votable", fileName,"/node");
    map.put("/node", itemInput);

    //Node - jcr:content
    JcrItemInput itemInputJcrContent = createNodeInput("nt:resource", null, "jcr:content","/node/jcr:content");
    map.put("/node/jcr:content", itemInputJcrContent);

    //properties - data
    JcrItemInput itemInputData = createPropertyInput("/node/jcr:content/jcr:data", "");
    map.put("/node/jcr:content/jcr:data", itemInputData);

    //properties - mimetype
    //mimetype can be: text/html,text/plain....
    JcrItemInput itemInputMimeType = createPropertyInput("/node/jcr:content/jcr:mimeType", mimeType);
    map.put("/node/jcr:content/jcr:mimeType", itemInputMimeType);

    //properties - last modified
    JcrItemInput itemInputLastMdf = createPropertyInput("/node/jcr:content/jcr:lastModified", cal);
    map.put("/node/jcr:content/jcr:lastModified", itemInputLastMdf);

    return map;
  }

  private JcrItemInput createNodeInput(String primaryType, String mixinTypes, String nodeName, String path) {
  ...
  private JcrItemInput createPropertyInput(String propertyPath,Object propertyValue) {
    JcrItemInput jcrItemInput = new JcrItemInput();
