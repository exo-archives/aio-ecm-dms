 function ECMUtils() {
  ...
  ECMUtils.prototype.insertContentToIframe = function(i) {
	...
	ECMUtils.prototype.tagKeyDown = function(event, clickedElement, position, option) {
    eXo.ecm.ECMUtils.itemPosition;
		if(window.event) // IE	
...
//		keychar = String.fromCharCode(keynum);
		if (keynum == 38) //UP_KEY 
			eXo.ecm.ECMUtils.itemPosition --;
		else if (keynum == 40) //DOWN_KEY
			eXo.ecm.ECMUtils.itemPosition ++;
		else if (keynum != 13) //ENTER_KEY
			eXo.ecm.ECMUtils.itemPosition = 0;
		else return false;
...
//		DOM.findDescendantById(tagNameInput, "names").setAttribute("autocomplete", "off");
		prefix = DOM.findDescendantById(tagNameInput, "names").value;
		// + keychar;
		prefix = prefix.toLowerCase();
//		alert(prefix);
		var prev = eXo.ecm.ECMUtils.itemPosition;
		itemList = DOM.findDescendantsByClass(showBlock, "div", "Item");
//		alert(itemList.length);
//		document.title = prefix;		
		var displayed = 0;
		for (var i = 0; i < itemList.length; i++) {
			item = itemList[i];
			tagName = item.getAttributeNode("value").value;
//			alert(tagName);
//			document.title = prefix;			
			if (tagName.toLowerCase().indexOf(prefix) == 0) {
		...
		if (eXo.ecm.ECMUtils.itemPosition < 0)
			eXo.ecm.ECMUtils.itemPosition = 0;
		if (eXo.ecm.ECMUtils.itemPosition >= displayed)
			eXo.ecm.ECMUtils.itemPosition = displayed - 1;
		realDisplayed = Math.min(displayed, 5);
		...
		for (var i = 0; i < itemList.length; i++) {
			...
			tagName = item.getAttributeNode("value").value;
			if (tagName.toLowerCase().indexOf(prefix) == 0) {
				if (count == eXo.ecm.ECMUtils.itemPosition) {
					item.style.backgroundColor="brown";
					if (keynum == 13) //ENTER_KEY
					{
						DOM.findDescendantById(tagNameInput, "names").value = tagName;
		...
		if(option == 1) {
			showBlock.style.width = (tagNameInput.offsetWidth / 2) + "px";
			showBlock.style.height = (realDisplayed * 20) + "px";
		}
		eXo.webui.UIPopup.show(showBlock) ;
		showBlock.onmousedown = function(event) {
			var event = event || window.event ;
		...
		Self.popupArray.push(showBlock);
		showBlock.style.top = tagNameInput.offsetHeight + 50 + "px";
		showBlock.style.left = 150 + "px";
		document.title = keynum + " " + realDisplayed + " " + eXo.ecm.ECMUtils.itemPosition;		
		if (eXo.ecm.ECMUtils.itemPosition >= realDisplayed)
			showBlock.scrollTop = ((eXo.ecm.ECMUtils.itemPosition - realDisplayed + 1) * 20);
		else if (eXo.ecm.ECMUtils.itemPosition < realDisplayed)
			showBlock.scrollTop = 0;
	};

	ECMUtils.prototype.enterTagName = function(event, clickedElement) {
		if(window.event) // IE	
		{	keynum = event.keyCode;	}
		else if(event.which) // Netscape/Firefox/Opera
		{	keynum = event.which;	}
//		keychar = String.fromCharCode(keynum);
		alert("Enter: " + clickedElement.getAttributeNode("value").value);
		if (keynum == 13) {
			popupSelector = DOM.findAncestorByClass(clickedElement, "UITaggingForm");
			tagNameInput = DOM.findFirstDescendantByClass(popupSelector,"div", "UITagNameInput");
			showBlock = DOM.findFirstDescendantByClass(popupSelector,"div", "UIFormTextAreaInput");
			...
			for (var i = 0; i < itemList.length; i++) {
				item = itemList[i];
				if (item.style.backgroundColor.equals("brown")) {
					DOM.findDescendantById(tagNameInput, "names").value = item.getAttributeNode("value").value;					
				}
			}
			showBlock.style.display="none";
			return false;			
	...
	ECMUtils.prototype.clickTagName = function(event, clickedElement) {
		if(window.event) // IE	
		{	keynum = event.keyCode;	}
		else if(event.which) // Netscape/Firefox/Opera
		{	keynum = event.which;	}
//		keychar = String.fromCharCode(keynum);
		popupSelector = DOM.findAncestorByClass(clickedElement, "UITaggingForm");
		tagNameInput = DOM.findFirstDescendantByClass(popupSelector,"div", "UITagNameInput");
		DOM.findDescendantById(tagNameInput, "names").value = clickedElement.getAttributeNode("value").value;
		showBlock = DOM.findFirstDescendantByClass(popupSelector,"div", "UIFormTextAreaInput");
		showBlock.style.display="none";
	};
	...
	ECMUtils.prototype.disallowEnter = function(event) {
		if(window.event) // IE	
		{	keynum = event.keyCode;	}
		else if(event.which) // Netscape/Firefox/Opera
		{	keynum = event.which;	}
