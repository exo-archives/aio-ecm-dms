...
public class TestFolksonomyService extends BaseDMSTestCase {
   ...
   * Input: name of tag style is nomal
   * Expected:
   ...
   * Input: there is no node to be added FFFF tag.
   * Expected: throws Exception.
   * 
   */
  ...
  public void testGetLinkedTagsOfDocument() throws Exception{
    Node root = session.getRootNode();
    Node document = root.addNode("document");
    session.save();
   ...
   * Input:
   *       htmlStyle: "font-size: 12px; font-weight: bold; color: #6b6b6b; font-family: verdana; text-decoration:none;"
   *       tagRate: "0..2"
   * Expected:
