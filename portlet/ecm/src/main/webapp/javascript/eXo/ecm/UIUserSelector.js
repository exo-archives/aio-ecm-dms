...
UIUserSelector.prototype.checkAll = function() {
	eXo.ecm.UIUserSelector.checkAllItem(this);
} ;
...
UIUserSelector.prototype.check = function() {
	eXo.ecm.UIUserSelector.checkItem(this);
} ;

UIUserSelector.prototype.checkAllItem = function(obj){
	var checked = obj.checked ;
	var items = eXo.ecm.UIUserSelector.getItems(obj) ;
...
UIUserSelector.prototype.checkItem = function(obj){
	var checkboxes = eXo.ecm.UIUserSelector.getItems(obj);
	var len = checkboxes.length;
	var state = true;
	if (!obj.checked) {
...
UIUserSelector.prototype.captureInput = function(input, action) {
  if(typeof(input) == "string") input = document.getElementById(input) ;
	input.form.onsubmit = eXo.ecm.UIUserSelector.cancelSubmit ;
  input.onkeypress= eXo.ecm.UIUserSelector.onEnter ;
} ;

UIUserSelector.prototype.onEnter = function(evt) {
  var _e = evt || window.event ;
  _e.cancelBubble = true ;
  var keynum = eXo.ecm.UIUserSelector.getKeynum(_e) ;
  if (keynum == 13) {
...
eXo.ecm.UIUserSelector = new UIUserSelector() ;