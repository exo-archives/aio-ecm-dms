...
public class PopupMenuListener extends UIComponent {
  ...
  public PopupMenuListener() throws Exception {
  }
  ...
  public String getNodeOwner() throws Exception { 
  ...
  public boolean nodeIsLocked(String path) throws Exception {
    UIJCRExplorer uiExplorer = getAncestorOfType(UIJCRExplorer.class) ;
    Node node ;
    if(path.indexOf(";") > -1) node = uiExplorer.getNodeByPath(path) ;
    else node = uiExplorer.getCurrentNode() ;
  ...
  public boolean isJcrViewEnable() throws Exception {
    if(getAncestorOfType(UIJCRExplorer.class).getPreference().isJcrEnable()) return true ;
    return false ;
  }
  
  private String getList(Node node, String path) throws Exception {
    StringBuilder actionsList = new StringBuilder() ;
    if(actionsList.length() > 0) actionsList.append(",") ;
    if(isReadAuthorized((ExtendedNode)node)) {
      if(isVersionableOrAncestor(node)) {
        if(node.isCheckedOut()) {
          ...
          if(hasRemovePermissions(node)) actionsList.append("RemoveNode") ;
        } else {
  ...
  static  public class EditDocumentActionListener extends EventListener<PopupMenuListener> {
    public void execute(Event<PopupMenuListener> event) throws Exception {
      PopupMenuListener uicomp = event.getSource() ;
      ...
      if(selectedNode.isNodeType("exo:action")) {
        ...
        uiPopupAction.activate(uiContainer, 600, 0) ;
      } else {
        ...
        if(documentNodeType.contains(nodeType)){
          ...
          UIPopupAction uiPopupAction = uiExplorer.getChild(UIPopupAction.class) ;
          uiPopupAction.activate(uiDocumentForm, 600, 0) ;
        } else {
  ...
  static  public class RenameActionListener extends EventListener<PopupMenuListener> {
    public void execute(Event<PopupMenuListener> event) throws Exception {
      PopupMenuListener uicomp = event.getSource() ;
      String renameNodePath = event.getRequestContext().getRequestParameter(OBJECTID) ;
      UIJCRExplorer uiExplorer = uicomp.getAncestorOfType(UIJCRExplorer.class) ;
      UIApplication uiApp = uicomp.getAncestorOfType(UIApplication.class) ;
      if(uicomp.nodeIsLocked(renameNodePath)) {
  ...
  static  public class CopyActionListener extends EventListener<PopupMenuListener> {
    public void execute(Event<PopupMenuListener> event) throws Exception {
      PopupMenuListener uicomp = event.getSource() ;
      String srcPath = event.getRequestContext().getRequestParameter(OBJECTID) ;
      UIJCRExplorer uiExplorer = uicomp.getAncestorOfType(UIJCRExplorer.class) ;
      UIApplication uiApp = uicomp.getAncestorOfType(UIApplication.class) ;
      if(uicomp.nodeIsLocked(srcPath)) {
        Object[] arg = { srcPath } ;
        uiApp.addMessage(new ApplicationMessage("UIPopupMenu.msg.node-locked", arg)) ;
        return ;
      }
...
//        uiExplorer.updateAjax(event) ;
  ...
  static  public class CutActionListener extends EventListener<PopupMenuListener> {
    public void execute(Event<PopupMenuListener> event) throws Exception {
      PopupMenuListener uicomp = event.getSource() ;
      UIJCRExplorer uiExplorer = uicomp.getAncestorOfType(UIJCRExplorer.class) ;
      String nodePath = event.getRequestContext().getRequestParameter(OBJECTID) ;
      UIApplication uiApp = uiExplorer.getAncestorOfType(UIApplication.class) ;
      if(uicomp.nodeIsLocked(nodePath)) {
        Object[] arg = { nodePath } ;
        uiApp.addMessage(new ApplicationMessage("UIPopupMenu.msg.node-locked", arg)) ;
        return ;
      }      
  ...
  static  public class RemoveNodeActionListener extends EventListener<PopupMenuListener> {
    public void execute(Event<PopupMenuListener> event) throws Exception {
      PopupMenuListener uicomp = event.getSource() ;
      UIJCRExplorer uiExplorer = uicomp.getAncestorOfType(UIJCRExplorer.class) ;
      String nodePath = event.getRequestContext().getRequestParameter(OBJECTID) ;
...
//        uiApp.addMessage(new ApplicationMessage("UIWorkingArea.msg.node-remove-success", args));
  ...
  static  public class LockActionListener extends EventListener<PopupMenuListener> {
    public void execute(Event<PopupMenuListener> event) throws Exception {
      PopupMenuListener uicomp = event.getSource() ;
      UIJCRExplorer uiExplorer = uicomp.getAncestorOfType(UIJCRExplorer.class) ;
  ...
  static  public class UnlockActionListener extends EventListener<PopupMenuListener> {
    public void execute(Event<PopupMenuListener> event) throws Exception {
      PopupMenuListener uicomp = event.getSource() ;
      UIJCRExplorer uiExplorer = uicomp.getAncestorOfType(UIJCRExplorer.class) ;
      String name = event.getRequestContext().getRequestParameter(OBJECTID) ;
  ...
  static  public class CheckInActionListener extends EventListener<PopupMenuListener> {
    public void execute(Event<PopupMenuListener> event) throws Exception {
      PopupMenuListener uicomp = event.getSource() ;
      UIJCRExplorer uiExplorer = uicomp.getAncestorOfType(UIJCRExplorer.class) ;
      String nodePath = event.getRequestContext().getRequestParameter(OBJECTID) ;
  ...
  static  public class CheckOutActionListener extends EventListener<PopupMenuListener> {
    public void execute(Event<PopupMenuListener> event) throws Exception {
      PopupMenuListener uicomp = event.getSource() ;
      UIJCRExplorer uiExplorer = uicomp.getAncestorOfType(UIJCRExplorer.class) ;
      String nodePath = event.getRequestContext().getRequestParameter(OBJECTID) ;
  ...
  static  public class CustomActionListener extends EventListener<PopupMenuListener> {
    public void execute(Event<PopupMenuListener> event) throws Exception {
      PopupMenuListener uicomp = event.getSource() ;
  ...
  static public class PasteActionListener extends EventListener<PopupMenuListener> {
    public void execute(Event<PopupMenuListener> event) throws Exception {
      PopupMenuListener uicomp = event.getSource() ;
      UIJCRExplorer uiExplorer = uicomp.getAncestorOfType(UIJCRExplorer.class) ;
      String nodePath = event.getRequestContext().getRequestParameter(OBJECTID) ;
      UIApplication uiApp = uiExplorer.getAncestorOfType(UIApplication.class) ;
      String[] arrPath = nodePath.split(";;") ;
      String srcPath = arrPath[0] ;
      String destPath = arrPath[1] ;
      ClipboardCommand currentClipboard = null ;
      try {
        for(ClipboardCommand clip : uiExplorer.getAllClipBoard()) {
          if(clip.getSrcPath().equals(srcPath)) { 
        ...
        Object[] arg = { nodePath } ;
        uiApp.addMessage(new ApplicationMessage("UIActionBar.msg.no-node-selected", arg));
        return ;
      }            
      ...
      if(uicomp.nodeIsLocked(destPath)) {
        Object[] arg = { destPath } ;
        uiApp.addMessage(new ApplicationMessage("UIPopupMenu.msg.node-locked", arg)) ;
        return ;
      }
